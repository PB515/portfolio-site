...
NEXT: **Phase 2 — public marketing shell** + the **Root/Canopy theme** from the mockups (update doc 04 + tokens to green+copper+cream; tree-root logo + "Engineer · Automate · Grow"; theme toggle). Build Home/About/Contact with IDENTITY-FIRST content (NO mockup copy/stats — those were AI filler). Then Phase 3 (admin CMS editors), Phase 4 (public DB reads + contact form), Phase 5 (polish).

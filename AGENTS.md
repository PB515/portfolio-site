...
**Phase 4 NEXT — make it public + wire forms.** Public /portfolio + /field-notes still show empty-states (they DON'T read the DB yet — that's the whole point of Phase 4): wire ISR reads of PUBLISHED rows, detail-by-slug, Markdown render (DEP — ASK: react-markdown or similar), About résumé link → current resume, contact form → server route (honeypot+validate+rate-limit → leads + email; needs ROTATED service-role key + email dep — ASK). Then Phase 5 polish (AA contrast both themes, SEO, metadataBase, analytics).
DECISION PENDING: deploy Phase 3 alone now (live admin, but public still empty until P4) vs build P4 then deploy together (published content shows live). Recommend P4-first.

...
**Phase 2 IN PROGRESS — shell + Root/Canopy theme + bg motif are LIVE** (merged to main, deployed Mumbai edge). Done: Root/Canopy tokens (04a; default Root, persisted "Perspective" toggle), (site) route group w/ SiteHeader (interim vector tree logo) + SiteFooter (real connect links) + MobileNav, identity-first Home, theme-aware SiteDecor (Root=copper circuit/roots, Canopy=sage leaves — one SVG recolored per theme). Components in `components/`. Login (positive path) confirmed working.
REMAINING Phase 2: About (real story→philosophy→skills arc→Experience[TBD]→resume link), Contact UI, restyle Portfolio/Field-Notes empty-states. THEN Phase 3 (admin CMS editors), Phase 4 (public DB reads + contact form wiring), Phase 5 (polish: AA contrast both themes, SEO, analytics).

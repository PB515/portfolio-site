...
**Phase 5 IN PROGRESS (branch `phase-5`).** DONE: Home hero text+image (real transparent cut-out portraits public/images/purven-{hero,about,contact}.png, framed copper); wordmark+tree-mark logo lockup (Logo.tsx + public/brand/logo-mark.png) + PNG favicon (app/icon.png, replaced code-gen icon.tsx); inline SVG icons (Icon.tsx); **AA contrast pass** — decoupled CTA copper into `--cta`/`--cta-hover` tokens (white text ≥4.5:1) vs link copper `--primary`; bigger nav (text-base) + admin chrome split (top nav / bottom footer w/ toggle); "Resume" (dropped é accents, UI text only — NOT routes/DB/bucket ids); SEO Person/Article JSON-LD + sitemap + metadataBase=live; @vercel/analytics (needs dashboard enable). **MOTION**: Reveal.tsx (IntersectionObserver scroll-reveal) + `.lift` hover + image zoom, gated behind prefers-reduced-motion. **Per-portrait art** (Portrait.tsx: glow/grid/rings CSS + user copper-on-black art via mix-blend-screen). **FULL 18-asset SVG DECOR SYSTEM** (docs/decor-asset-spec.md): user-generated theme-aware SVGs in public/decor/{root,canopy}/ → inlined as token-recoloured components (SideDecor gutters, Hero/Lower/Footer/About/Contact/Portfolio/FieldNotes/Admin Decor + Decor/Motif); `.decor-root`/`.decor-canopy` CSS swaps per data-theme. PENDING: shrink og-default.png (5.2MB!) + favicon icon.png (969KB) externally; 360px pass.

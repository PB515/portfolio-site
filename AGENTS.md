...
**Phase 3 IN PROGRESS — admin CMS** on the proven-secure foundation. Order: data helpers → Categories → Projects → Field Notes → Resume → cover/resume Storage upload → RE-RUN denial gate after writes. Admin CRUD uses the logged-in SESSION (anon key + RLS `authenticated` policies); service-role key only for contact insert (Phase 4). Editor body = Markdown textarea first (no dep); WYSIWYG later (dep approval).
THEN Phase 4 (public DB reads via ISR + contact form wiring), Phase 5 (polish: AA contrast both themes, SEO, metadataBase, analytics).
